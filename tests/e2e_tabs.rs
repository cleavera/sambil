...
/// Ctrl-b c opens a new tab, switches to it, and shows it in the tab bar.
#[test]
...
fn can_open_multiple_tabs() {
    ...
    session.send_keys(&[CTRL_B, b'c']);
    assert!(session.wait_for_text("New tab name:", Duration::from_secs(2)), "prompt 1 did not appear");
    session.send_str("\r");
    assert!(session.wait_for_text("[*2:", Duration::from_secs(2)), "tab 2 did not open");

    session.send_keys(&[CTRL_B, b'c']);
    assert!(session.wait_for_text("New tab name:", Duration::from_secs(2)), "prompt 2 did not appear");
    session.send_str("\r");
    assert!(
