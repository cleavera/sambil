...
fn tmp_config_dir(label: &str) -> std::path::PathBuf {
    ...
    let dir = std::env::temp_dir()
        .join(format!("sambil_{}_{}_{}_{}", label, std::process::id(), n, "cfg"));
...
fn config_file_created_on_first_launch() {
    ...
    assert!(content.contains("leader"), "config file is missing the leader setting");
    assert!(content.contains("ctrl+b"), "config file should default to ctrl+b");
